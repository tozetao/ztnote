...
首先，你必须创建一个扩展类，就像[如何在捆绑中加载服务配置](https://symfony.com/doc/5.4/bundles/extension.html)中所解释的那样。
...
每当用户在配置文件中包含`acme_social`键（也就是DI别名），它下面的配置就会被添加到一个配置数组中，并传递给你的扩展的`load()`方法（Symfony自动将XML和YAML转换为数组）。
...
不过不用担心! Symfony的Config组件将帮助你合并这些值，提供默认值，并在配置不良时给用户提供验证错误。下面是它的工作原理。在DependencyInjection目录下创建一个Configuration类，并建立一棵树，定义你的bundle的配置结构。
...
与其每次提供一些配置选项时在你的扩展中调用`processConfiguration()`，不如使用[ConfigurableExtension](https://github.com/symfony/symfony/blob/5.4/src/Symfony/Component/HttpKernel/DependencyInjection/ConfigurableExtension.php)来自动为你做这个：
...
如果你有多个相互依赖的捆绑包，允许一个扩展类修改传递给另一个捆绑包的扩展类的配置可能是有用的。这可以通过一个prepend扩展来实现。更多细节，请参见[如何简化多个捆绑包的配置]（https://symfony.com/doc/5.4/bundles/prepend_extension.html）。
...
只要你的捆绑包的配置位于标准位置（`YourBundle/DependencyInjection/Configuration`）并且没有构造函数，它就会自动工作。如果你有不同的东西，你的扩展类必须覆盖[Extension::getConfiguration()](https://github.com/symfony/symfony/blob/5.4/src/Symfony/Component/DependencyInjection/Extension/Extension.php#method_getConfiguration)方法并返回你的`Configuration'实例。
