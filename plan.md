to do list

实践一个高性能的抢购系统

实现一个聊天系统

网络编程实践，编写一个web服务器

实践风格风格良好的API接口



学习目标

一定的数学知识

一定的算法能力

一定的英语水平



### 推荐书籍

- linux/unix系统编程手册
- linux高性能服务器编程
- TCP/IP高效编程（Effective TCP/IP Programing）
- 图解TCP/IP详解
- unix网络编程
- 现代化PHP（Model PHP）
- 高性能MySQL



### 学习资料

斯坦福公开课

清华大学公开课





编程理论

http://open.163.com/special/opencourse/abstractions.html
http://open.163.com/special/opencourse/paradigms.html
http://open.163.com/special/opencourse/bianchengdaolun.html
http://open.163.com/special/programming/



数学

http://open.163.com/special/opencourse/daishu.html

http://open.163.com/special/opencourse/equations.html

http://open.163.com/special/opencourse/multivariable.html

http://open.163.com/special/sp/singlevariablecalculus.html



拿证的网址

https://www.coursera.org/specializations/algorithms

https://www.coursera.org/



### 学习清单

- Unix网络编程卷1  => linux高性能服务器编程 => C Primer Plus => C语言接口与实现
- 有明（Laravel）=> 秒杀功能的实现 (IOC/PSR标准/Restful) => 凯威的讲堂
- MySQL实战45讲 => Linux性能优化实战 => 玩转git三剑客 => Redis（掘金） =>  Nginx核心100讲 => 趣谈网络协议 => 程序员的数学基础课 => 数据结构与算法之美



> 持续性任务

- 整理面试题 => 解决面试题



> 当前任务

- 有明（Laravel）

  学习日志处理、错误处理、参数异常处理与验证，编写风格良好的代码

- unix网络编程

  开始于2019年1月，目前只是看了重点章节

- Linux高性能编程

  开始于2019年2月中旬，看完后需要做一个总结。

- MySQL实战45讲

  任务起始时间：2018年12月

- 实践一个抢购系统

  学习处理高并发下的资源抢用，如何提高QPS

