...
派生选择器允许定位到一个标签，通过标签的层级关系，来定义子元素的样式。
```html
<style>
	.content{
		width: 200px;
...
```
在上面的例子中，span标签未定义任何样式，但是继承了body标签的样式，所以字体是红色的。
而div标签定义了字体样色样式，所以是yellow颜色，h1标签又重新定义了字体样色样式，所以对div、body标签样式都进行的覆盖。

上面的例子是标签样式继承的重叠，在多个选择器同时作用于一个标签时，样式也可能会发生重叠，这里涉及到不同选择器样式采用的优先级问题，例如：
```html
<style type="text/css">
	.first{
		color: yellow;
...
优先级说明：
优先级的计算是按照选择器所对应的分数来计算的，一个标签选择器是1分，一个类选择器是10分，一个id选择器是100分，一个行内选择器是1000分。如果样式不同，那么样式会组合使用，如果样式重叠，那么会优先使用分高的样式。

注1：如果选择器是相同的，会按照选择器定义的先后顺序来采用样式
